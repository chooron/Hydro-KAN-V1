...
"""
    ...
    acts31_output = q_func_131(x[3], p)
...
function plot_q_symbolize_fit(basin_id)
    ...
    # Combine plots
    layer1_plot = plot(
        fig111, fig121, fig131, fig112, fig122, fig132, framestyle=:box,
        layout=(2, 3), size=(1200, 300), legend=false, fontcolor=:black,
        fontfamily="Times", tickfont=font(14, "Times"), dpi=300
    )

    layer2_plot = plot(
        fig211, fig221, framestyle=:box,
        layout=(2, 1), size=(1200, 300), legend=false, fontcolor=:black,
        fontfamily="Times", tickfont=font(14, "Times"), dpi=300
    )
...
struct QNNSymbol2 <: LuxCore.AbstractLuxLayer
    name::Symbol
end

LuxCore.initialparameters(::AbstractRNG, ::QNNSymbol2) = reduce(merge, (params_121, params_131, params_122, params_211, params_221))
LuxCore.initialstates(::AbstractRNG, ::QNNSymbol2) = NamedTuple()

function (::QNNSymbol2)(x::AbstractVector, p, st)
    acts21_output = q_func_121(x[2], p)
    acts31_output = q_func_131(x[3], p)
    acts22_output = q_func_122(x[2], p)
    ...
    return [q_func_211(layer1_output, p) + q_func_221(layer2_output, p)], st
end

function (::QNNSymbol2)(x::AbstractMatrix, p, st)
    acts21_output = q_func_121.(x[2, :], Ref(p))
    acts31_output = q_func_131.(x[3, :], Ref(p))
    acts22_output = q_func_122.(x[2, :], Ref(p))
    layer1_output = clamp.(acts21_output .+ acts31_output, layer2_input_extrema1...)
    layer2_output = clamp.(acts22_output, layer2_input_extrema2...)
    output1 = q_func_211.(layer1_output, Ref(p))
    output2 = q_func_221.(layer2_output, Ref(p))
    return reshape(output1 .+ output2, 1, :), st
end

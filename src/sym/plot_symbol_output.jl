...
function plot_figure(basin_id, show_ylabel=true)
    ...
    nse_symbolize, mnse_symbolize, fhv_symbolize, rmse_symbolize = nse(obs, pred_symbolize), mnse(obs, pred_symbolize), fhv(obs, pred_symbolize), rmse(obs, pred_symbolize)
    nse_exphydro, mnse_exphydro, fhv_exphydro, rmse_exphydro = nse(obs, pred_exphydro), mnse(obs, pred_exphydro), fhv(obs, pred_exphydro), rmse(obs, pred_exphydro)
    nse_k50, mnse_k50, fhv_k50, rmse_k50 = nse(obs, pred_k50), mnse(obs, pred_k50), fhv(obs, pred_k50), rmse(obs, pred_k50)
...
savefig(p, "src/plots/figures/symbol_output.png")
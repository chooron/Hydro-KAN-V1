...
function plot_activation_functions(acts, postacts, colors=[:skyblue, :salmon, :mediumseagreen])
    ...
    # 创建一个网格布局的图
    p = plot(layout=(activ_dims, input_dims), bottom_margin=5Plots.mm,
    ...
    for i in axes(acts, 1)
        ...
        for j in axes(postacts, 2)
            ...
            plot!(
                ...
                linewidth=4, color=colors[i], left_margin=i == 1 ? 0Plots.mm : 5Plots.mm # 5 for 01137500
            )
            scatter!(
                p[plot_idx],
                current_acts[sorted_indices[length(sorted_indices)-10:end]], current_postacts[sorted_indices[length(sorted_indices)-10:end]],
    ...
    # 调整整体布局
    plot!(p, fontfamily="Times", framestyle=:box, grid=true, margin=0.1Plots.mm)
    return p
end
...
function plot_one_xai_scatter_m50!(fig, basin_id, var_type, color=:red)
    ...
    return scatter!(fig, x, y, alpha=0.8, color=color,
...
p2 = plot_activation_functions(qnn_layer1_output, qnn_layer2_postacts_arr, [:mediumpurple])
xaxis!(p2, xticks=[0, 2, 4, 6, 8, 10])
p_layout = @layout [grid(1, 2, widths=[3 / 4, 1 / 4])]
p = plot(p1, p2, layout=p_layout, fontfamily="Times", leftmargin=2Plots.mm,
    legend=false, framestyle=:box, grid=true, dpi=300, tickfontsize=14
)
...
scatter_list = map(1:3) do i
    scatter(qnn_input[i, indices], expl.val[i, :], alpha=0.8, color=scatter_colors[i],
        tickfontsize=14, leftmargin=4Plots.mm, markerstrokewidth=1.0,
        markerstrokecolor=:black)
end
scatter_plot = plot(
...
m50_xai_plot = plot(fig1, fig2, fig3, layout=(1, 3), size=(900, 300), dpi=300)

#! m50_xai_plot is in plot_special_xai.jl
fig = plot(
    p, scatter_plot, m50_xai_plot, layout=(3, 1), size=(1000, 800), tickfontsize=14, margin=0.1Plots.mm,
